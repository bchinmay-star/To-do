...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    private static final String CREATE_COMPLETE_TABLE = "CREATE TABLE " + COMPLETE_TABLE + "(" + ID + " INTEGER PRIMARY KEY AUTOINCREMENT, " + TASK + " TEXT, " + STATUS + " INTEGER)";
    ...
    public void insertCompleteTask(ToDoModel task){
        ContentValues cv = new ContentValues();
        cv.put(TASK, task.getTaskName());
        cv.put(STATUS, 1);
        db.insert(COMPLETE_TABLE, null, cv);
    }
    ...
    public List<ToDoModel> getAllCompleteTasks(){
        List<ToDoModel> taskList = new ArrayList<>();
        Cursor cur = null;
        db.beginTransaction();
        try{
            cur = db.query(COMPLETE_TABLE, null, null, null, null, null, null, null);
            if(cur != null){
                if(cur.moveToFirst()){
                    do{
                        ToDoModel task = new ToDoModel();
